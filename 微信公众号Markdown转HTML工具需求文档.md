...
#### 2.2.3 样式导入/导出（CSV/JSON）
- **CSV 格式导入**：
  - 支持 CSV 文件上传
  - CSV 格式规范（包含样式参数列）：
    ```csv
    styleName,backgroundColor,textColor,fontSize,lineH1Size,h1Color,quoteBgColor,...
    我的自定义样式,#ffffff,#333333,16,24,#2c3e50,#f5f5f5,...
    ```
  - 导入后验证样式参数有效性
  - 显示导入成功/失败提示
...
```
┌─────────────────────────────────────────────────────┐
│  工具标题 + 样式选择器 [▼] + [配置样式] [导入/导出]  │
├─────────────────────┬───────────────────────────────┤
│                     │                               │
│   Markdown 编辑器    │        HTML 预览              │
│  (实时预览模式)      │     (渲染效果)                │
│                     │                               │
│  [清空] [导入.md]    │   [复制HTML] [复制富文本]      │
├─────────────────────┴───────────────────────────────┤
│              HTML 源码显示区                        │
│              [一键复制]                             │
└─────────────────────────────────────────────────────┘
...
3. **CSV 导入样式**：CSV 格式是否必要？或者只提供 JSON 导入导出？（建议：以 JSON 为主，CSV 为可选功能）
4. **富文本复制功能**：是否需要直接复制为富文本（可直接粘贴到微信公众号编辑器）？（建议：Phase 3 实现）
5. **图片处理**：是否需要支持图片上传功能？（建议：暂时不支持，只保留图片链接）
6. **后端 API**：是否需要后端支持？或者全部前端处理？（建议：全部前端处理，不依赖后端）
